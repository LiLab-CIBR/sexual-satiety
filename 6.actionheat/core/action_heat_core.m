...
cuttime(1) = neuron.events{1,actionidx{1}}(find(neuron.events{1,actionidx{1}}(:,1) > neuron.intruder(cuelist(1),1),1),1);
cuttime(2) = neuron.events{1,actionidx{2}}(mountidx,1)+movetime;
if ~isempty(actionidx{4})
    cuttime(3) = neuron.events{1,actionidx{4}}(find(neuron.events{1,actionidx{4}}(:,1) > neuron.intruder(cuelist(1),1),1),1);
end
