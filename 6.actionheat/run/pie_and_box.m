...
% 找第一个有ejaculate的cue,去除后续female cue
        intruderl = neuron.intruder_label;
        for i = cuelist
            if ~isempty(neuron.intruder_action{i,actionidx{4}})||contains(stateslist{f},'St1817')
                cuechoose = i;
...
saveas(gcf,['E:\wupeixuan\auc_plot\data\dzyimg\miniscope data plot\Figure1-3\fig2update\0521materials\pie_actioncut.pdf']);
...
for p =1:length(stateslist)
    xlswrite([savedir,'\datafile\figs5_efjk_box2.xlsx'],{'appe','cons'},stateslist{p},'A1')
    xlswrite([savedir,'\datafile\figs5_efjk_box2.xlsx'],meanappe{p},stateslist{p},[idxlist(1),'2'])
    xlswrite([savedir,'\datafile\figs5_efjk_box2.xlsx'],meancons{p},stateslist{p},[idxlist(2),'2'])
...
hfig.Renderer = 'Painters';                                                   
hfig.PaperSize = [50,20];
saveas(gcf,['E:\wupeixuan\auc_plot\data\dzyimg\miniscope data plot\Figure1-3\fig2update\0521materials\box_actioncut.pdf']);
