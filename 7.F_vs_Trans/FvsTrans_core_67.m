...
function FvsTrans_core_67(fvstrans)
...
for i = 1:length(animallist)
    for j = 1:length(aa{i})
        ...
        actionidx{i}{j}{2} = find(contains(neuron{i}{j}.action_label,'mount'));
        if length(actionidx{i}{j}{2}) >1
        ...
        [num_neuron, nframe] = size(dec_data{i}{j});
        tlen{i}{j} = nframe ;
...
%     saveas(gcf,['E:\wupeixuan\auc_plot\data\dzyimg\miniscope data plot\Figure1-3\fig2update\0420materials\',...
