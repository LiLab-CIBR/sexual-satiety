function aucPlot_ver2(result_folder, load_neuron, load_auc, Fs, gender, thresh,intruder_label,trace_per_plot,double_thresh,first_event,second_event, auc_thresh)
...
% aucPlot('E:\wupeixuan\auc_plot\data\Esr24f\Esr240127',false, false,10,1,0.99,{},18,0.8,2,6,[0.7 0.3])
% result_folder = 'E:\wupeixuan\auc_plot\data\auc2 series\Esr222\hide\Esr2220621';
% load_neuron = false;
% load_auc = false;
% Fs = 10;
% gender = 1;
...
%         color = {[1 0 0],[1 0.63 0],[0 1 0],[0.494 0.184 0.556],[0 0 1],[0 1 1], [1 1 0],[1 0.62 0.83], [1 0.5 0.5], [0.46 1 1], [1 0.155 0.155], [0.15 1 0.15],[0.05 0.749 0.549],[0.46 0.078 0.57],[1 0.75 0.80]};  %approach [0.46 0.078 0.57]  [0.2 1 0.597]
        ...
        action_label = {'positive','mounting','attack','biting','retrieve','beaten','sniff other','sniff pups at nest', 'feeding','mounted','intromitted','ejaculated','fleeing','sitting','passive','lordosis'}; % passive,[1 0.63 0] chase [1 0.75 0.80]
        color = {[1 0 0],[0.4940 0.1840 0.5560],[0 1 1], [1 1 0],[1 0.62 0.83], [1 0.5 0.5], [1 0.155 0.155], [0.15 1 0.15], [0.82 0.41 0.12],[0.05 0.749 0.549],[0.2 0.597 1],[0.796 0.598 1], [1 0.4 1],[1 1 0],[1 0.63 0],[0.46 0.078 0.57]};  %[0.46 0.078 0.57]  [0.2 1 0.597]
    end
...
flag = revertTTL2bin(usedlist(1), 180, neuron.Fs, size(data,2));
firstbase = data(:,flag);
nbtrace = (data-mean(firstbase,2))./std(firstbase,0,2);

%% plot deconvolution data
[dec_ind, dec_seg_ind] = thresh_order(nbtrace,neuron,thresh,trace_per_plot);
% calculate the mean trace
% mean_trace = zeros(10,neuron.nframe);
% mean_trace(2,:) = mean(dec_data,1);d
% dec_seg_ind = {1:10};
% dec_data = mean_trace;
plot_data(nbtrace,neuron,dec_seg_ind,'deconv','plot_thresh',thresh,'zeroline','save_path',result_folder)
% 
...
if ~exist('auc_result1','var') || ~load_auc
    auc_result1 = auc_cue(nbtrace,sig_data,neuron,auc_thresh(1),result_folder,filelist,'light_on');
    plot_data(nbtrace,neuron,dec_seg_ind,'auc',auc_result1,'zeroline','zero_blank');
end
...
if ~exist('auc_result2','var') || ~load_auc
    auc_result2 = auc_cue(nbtrace,sig_data,neuron,auc_thresh(2),result_folder,filelist,'light_on');
    plot_data(nbtrace,neuron,dec_seg_ind,'auc',auc_result2,'zeroline','zero_blank');
end
