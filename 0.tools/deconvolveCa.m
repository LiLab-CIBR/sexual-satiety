...
%       b:  fluorescence baseline values. default is 0
...
%             [c, s] = onnls(y-options.b, options.pars, options.lambda, ...
%                 options.shift, options.window, options.maxIter);
            [c, s] = onnls(y-options.b, options.pars, options.lambda, options.shift, options.window);
        else
            disp('to be done');
        end
    case 'constrained'
