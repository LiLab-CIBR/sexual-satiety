...
if beh.isdivide
    for i = 1:size(events,2)
        ...
        saveas(gcf,[savedir,'\fig1_behavior',name,'_',clusterlist{i},'_divided.pdf']);
    end
else
    for i = 1:size(events,2)
        ylabels{i} = {};
...
%         xlim([0,xlength])
        ...
        for j = 1:size(events{i},2) 
            ...
            ylabels{i} = [ylabels{i},datelist{i}{j}(1:4)];
        end
        yticklabels(ylabels{i});
        hfig.Renderer = 'Painters';
        hfig.PaperSize = [35,50];
        saveas(gcf,[savedir,'\fig1_behavior_',name,'_',clusterlist{i},'_inone.pdf']);
    end
end
