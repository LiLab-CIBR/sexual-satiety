function [transmatrix,pvalue,newbehaviorlist,cmatrix] = DrawTranstion(transition)
...
for i = 1:length(clusterlist)
    ...
    saveas(gcf,[savedir,'\picfile\fig1_transition_map_',name,clusterlist{i},'.pdf']);
...
if is_ttest_only
    for ss = 1:length(ttest_group)
        ...
        hfig.Renderer = 'Painters';
        hfig.PaperSize = [30,30];
        saveas(gcf,[savedir,'\fig1_transition_',name,'_ttest.pdf']);
        % p值
        pic2 = figure();
        set(gcf,'Position',[100,100,500,550])
        ...
        hfig.Renderer = 'Painters';
        hfig.PaperSize = [30,30];
        saveas(gcf,[savedir,'\picfile\fig1_transition_',name,'_ttest_p.pdf']);
    ...
    if length(clusterlist) == 2    %ttest
        ...
        hfig.Renderer = 'Painters';
        hfig.PaperSize = [30,30];
        saveas(gcf,[savedir,'\picfile\fig1_transition_',name,'_ttest.pdf']);
    % p值
        pic2 = figure();
        set(gcf,'Position',[100,100,500,550])
        dist = heatmap(xvalues,yvalues,ttestmatrix{2});
    ...
    elseif length(clusterlist) > 2
        ...
        hfig.Renderer = 'Painters';
        hfig.PaperSize = [30,30];
        saveas(gcf,[savedir,'\picfile\fig1_transition_',name,'_anova_p.pdf']);
        pvalue{1} = stanova;
    end
end
idxlist = 'ABCDEFGHIJK';
