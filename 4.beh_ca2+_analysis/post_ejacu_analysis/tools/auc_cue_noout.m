function [auc_result, neuron] = auc_cue_noout(dec_data,sig_data,neuron,auc_thresh,varargin)
...
for i = 1:nstate
    ...
    for j=1:nintruder
        ...
        lightoff = light(find(light(:,2)>=cue_now(2),1),2);
        if lightoff > tlen
            lightoff = tlen;
        end
        cue_dur = lightoff - cue_now(1)-10;
        if j<nintruder
            if intruder(j+1,2)< lightoff
...
for j=1:nintruder
    ...
    for i=1:nstate
        ...
        if strcmp(neuron.action_label{i},'positive')||contains(neuron.action_label{i},'postejact')
            baselines(j,i,:) = zerostates(j,:);
%         elseif contains(neuron.action_label{i},'postejact')
%             baselines(j,i,:) = zerostates(j,:);
        else
...
%% calculate auROC
ejact_idx = find(ismember(neuron.action_label,'ejacu'));
tic;
...
for iintruder = 1:nintruder
    ...
    %     zerostate = zerostates>0;
        ...
        if contains(neuron.action_label{istate},'newb')
            behavioralstate = bl;
        end
